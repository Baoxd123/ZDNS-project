...
type Config struct {
	...
	PrometheusLA       string          `long:"prometheus" description:"(DEBUG Local Addr)Address to use for Prometheus server (e.g. localhost:8080). If empty, Prometheus is disabled."`
...
func validateFrameworkConfiguration() {
	...
	if config.LocalAddrStr != "" {
		...
		log.Infof("Resolved local address to: %s", config.localAddr.String())
		if config.Debug {
			log.Debugf("Debug: Resolved local address to: %s", config.localAddr.String())
		}
	}
	...
	// validate/start prometheus
	if config.PrometheusLA != "" {
		var err error
		config.localAddr, err = net.ResolveTCPAddr("tcp", config.PrometheusLA)
		if err != nil {
			log.Fatalf("could not resolve local address %s: %v", config.PrometheusLA, err)
		}
		log.Infof("Resolved local address to: %s", config.localAddr.String())
		if config.Debug {
			log.Debugf("Debug: Resolved local address to: %s", config.localAddr.String())
